...
function orderToPdf(){
    ...
    $orderTemplate = "";
    //テンプレート読み込む
    if(!$orderTemplate = file_get_contents("./files/Tmstemplate.html")){
  ...
  //-------------------------------------------------この中のコードは2014-10-16に増加します---------------------------------------------------
    ...
    $res_order = mysql_query($sql_order, $db) or die("DB putout error4");
    ...
    while ($row_order = mysql_fetch_array($res_order,MYSQL_ASSOC)){
    	...
    	$build_date=date("Y",time($row_order["BuildDate"])).'年'.date("n",time($row_order["BuildDate"])).'月'.date("d",time($row_order["BuildDate"])).'日';
    	$orderTemplate=preg_replace('/{##'.buildDate.'}/',$build_date, $orderTemplate);
    }
    //workテーブルから
   $sql_work = "SELECT `workContents`, `number`, `unitPrice`, `price`, `addunitprice`, `subtractunitprice` FROM `work` Where `saibanRes`='" . $_POST["saibanRes"] . "' ORDER BY `id`";
    ...
    if(strstr($orderTemplate, "{!") && strstr($orderTemplate, "!}")){
        ...
        while ($row_work = mysql_fetch_array($res_work, MYSQL_ASSOC)) {
                ...
                $listTplTr = preg_replace('/{\$price}/', $row_work["price"], $listTplTr);
                $listTplTr = preg_replace('/{\$addunitprice}/', number_format($row_work["addunitprice"]), $listTplTr);
                $listTplTr = preg_replace('/{\$subunitprice}/', number_format($row_work["subtractunitprice"]), $listTplTr);
                $sumprice=$sumprice+$row_work["price"];
                $shz=$sumprice*0.08;
                $sumpricez=$sumprice+$shz;
