...
function TmsorderToPdf(){
    ...
    $orderTemplate = "";
    //テンプレート読み込む
    if(!$orderTemplate = file_get_contents("../files/newTmstemplate.html")){
  ...
  //-------------------------------------------------この中のコードは2014-10-16に増加します---------------------------------------------------
    ...
    //workテーブルから
   $sql_work = "SELECT `id`,`workContents`,`addunitprice`, `number`,`unitPrice`,`subtractunitPrice`,`Price` FROM `work` Where `saibanRes`='" . $_POST["saibanRes"] . "' ORDER BY `id`";
    ...
    if(strstr($orderTemplate, "{!") && strstr($orderTemplate, "!}")){
        ...
        while ($row_work = mysql_fetch_array($res_work, MYSQL_ASSOC)) {
                ...
                $listTplTr = preg_replace('/{\$subtractunitprice}/', number_format($row_work["subtractunitPrice"]), $listTplTr);
