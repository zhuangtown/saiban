...
function orderToPdf(){
  ...
  //-------------------------------------------------この中のコードは2014-10-16に増加します---------------------------------------------------
    ...
    //workテーブルから
   $sql_work = "SELECT `id`,`workContents`,`subtracttime`,`addtime`, `time`,`unitPrice`,`addPrice`,`trance`,`Price` FROM `work` Where `saibanRes`='" . $_POST["saibanRes"] . "' ORDER BY `id`";
    ...
    if(strstr($orderTemplate, "{!") && strstr($orderTemplate, "!}")){
        ...
        while ($row_work = mysql_fetch_array($res_work, MYSQL_ASSOC)) {
                ...
                $sumprice=$sumprice+$row_work["Price"]+$row_work["addPrice"];
