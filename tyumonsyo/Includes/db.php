...
class WishDB extends mysqli {
    ...
    public  function get_db_count($Name){
    	$name = $this->real_escape_string($name);
    	$count = $this->query("select count(*) from ".$name);
    	return $count;
    }
    
    public function get_wisher_id_by_name($name) {
        $name = $this->real_escape_string($name);
        $wisher = $this->query("SELECT code FROM customer WHERE name = '"
                        . $name . "'");
    ...
    public function delete_wish($wishID) {
    	$this->query("DELETE FROM customer WHERE id = " . $wishID);
    }
